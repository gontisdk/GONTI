...
void gontiVkSyncObjectsCreate(GontiVulkanContext* context) {
    ...
    context->imageAvailableSemaphores = darrayReserve(VkSemaphore, context->swapchain.imageCount);
    ...
    for (u32 i = 0; i < context->swapchain.imageCount; i++) {
        ...
        VkResult result1 = vkCreateSemaphore(context->device.logicalDevice, &semaphoreCreateInfo, 
            context->allocator, &context->imageAvailableSemaphores[i]);
        VkResult result2 = vkCreateSemaphore(context->device.logicalDevice, &semaphoreCreateInfo, 
            context->allocator, &context->queueCompleteSemaphore[i]);

        if (result1 != VK_SUCCESS || result2 != VK_SUCCESS) {
            KERROR("Failed to create semaphores for image %d", i);
            return;
        }

        KDEBUG("Created semaphores for image %d: ImageSem=%p, QueueSem=%p", 
    ...
    KINFO("Sync objects created successfully - %d image semaphores, %d frame fences", 
        context->swapchain.imageCount, context->swapchain.maxFramesInFlight);
}
