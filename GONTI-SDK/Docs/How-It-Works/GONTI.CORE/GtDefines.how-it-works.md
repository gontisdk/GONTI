...
`GtDefines.h` is the foundational header of the entire `GONTI.CORE` library. It is a **header-only** file included by virtually every other module. Its purpose is to establish a stable, portable, and consistent type system, platform detection layer, compiler abstraction, and a set of utility macros used throughout the engine.
...
`GtDefines.h` has **no dependencies** on any other engine header. It is the root node of the entire `GONTI.CORE` include graph.

```
GtDefines.h   ←   (no engine dependencies)
     ↑
